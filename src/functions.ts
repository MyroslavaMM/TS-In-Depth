...
export function getBookAuthorByIndes(index: number): [title: string, author: string] {
    const books = getAllBooks();
...
export function asserRerBookInstance(condition: any): asserts condition {
    if (!condition) {
...
export function printRefBook(data: any): void {
    asserRerBookInstance(data instanceof RefBook);
    data.printItem();
}
...
export function getBookByCategoryPromise(category: Category): Promise<string[]> {
    const p: Promise<string[]> = new Promise((resolve, regect) => {
        setTimeout(() => {
            const titles = getBookTitlesByCategory(category);
